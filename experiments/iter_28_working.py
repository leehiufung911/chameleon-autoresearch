...
sys.stderr.write("STEP 1: Starting\n")
sys.stderr.flush()

import numpy as np

sys.stderr.write("STEP 2: NumPy imported\n")
sys.stderr.flush()

from rdkit import Chem

sys.stderr.write("STEP 3: RDKit imported\n")
sys.stderr.flush()

from rdkit.Chem import AllChem, Descriptors

sys.stderr.write("STEP 4: RDKit modules imported\n")
sys.stderr.flush()

import json

sys.stderr.write("STEP 5: JSON imported\n")
sys.stderr.flush()
...
sys.stderr.write("STEP 6: Functions defined\n")
sys.stderr.flush()
...
sys.stderr.write(f"STEP 7: Loaded {len(protacs)} PROTACs\n")
sys.stderr.flush()
...
for name, smiles in protacs:
    ...
    if result:
        ...
        output_lines.append(f"  {label}: Cooperativity = {result['cooperativity']:.3f}")
        sys.stderr.write(f"  {label}: Cooperativity = {result['cooperativity']:.3f}\n")
        sys.stderr.flush()
...
sys.stderr.write("Writing output files...\n")
sys.stderr.flush()
