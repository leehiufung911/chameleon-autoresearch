import os

os.environ["PYTHONUNBUFFERED"] = "1"

import sys

sys.path.insert(
    0, "C:/Users/mic23/prototype-embed/chameleon-research-loop/chameleon_local"
)

from rdkit import Chem
import json
import numpy as np

# Output buffer
output_lines = []


def log(msg):
    output_lines.append(str(msg))
    print(msg, flush=True)


log("=" * 80)
log("ITERATION 22: Fragment-Based Chameleonicity Prediction")
log("=" * 80)
log("")

# SMARTS patterns for fragment analysis
PROMOTING = {
    "peg_chain": "[OX2][C;!R][C;!R][OX2]",  # PEG-like pattern
    "ether": "[OX2]-[!H]",  # Ether oxygens
    "amide": "[NX3](C=O)",  # Amide
    "ester": "C(=O)[OX2]",  # Ester
}

INHIBITING = {
    "long_alkyl": "[C;!R]-[C;!R]-[C;!R]-[C;!R]-[C;!R]",  # 5+ carbon chain
    "longer_alkyl": "[C;!R]-[C;!R]-[C;!R]-[C;!R]-[C;!R]-[C;!R]",  # 6+ carbon chain
}


def count_matches(mol, patterns):
    """Count substructure matches for each pattern."""
    counts = {}
    for name, smarts in patterns.items():
        try:
            pat = Chem.MolFromSmarts(smarts)
            if pat:
                matches = mol.GetSubstructMatches(pat)
                counts[name] = len(matches)
            else:
                counts[name] = 0
        except Exception as e:
            counts[name] = 0
    return counts


def compute_fragment_score(mol):
    """Compute chameleon score based on fragment analysis."""
    prom = count_matches(mol, PROMOTING)
    inhib = count_matches(mol, INHIBITING)

    # Weighted scores
    prom_score = (
        prom.get("peg_chain", 0) * 3.0
        + prom.get("amide", 0) * 2.0
        + prom.get("ether", 0) * 1.0
        + prom.get("ester", 0) * 0.5
    )

    inhib_score = inhib.get("long_alkyl", 0) * 2.0 + inhib.get("longer_alkyl", 0) * 3.0

    total = prom_score + inhib_score
    balance = prom_score / total if total > 0 else 0.5

    # Chameleon score: balance weighted by fragment richness
    score = balance * (1 + np.log1p(prom_score)) / (1 + np.log1p(inhib_score + 1))

    return {
        "promoting_score": prom_score,
        "inhibiting_score": inhib_score,
        "balance": balance,
        "chameleon_score": float(score),
        "prom_details": prom,
        "inhib_details": inhib,
    }


def load_user_protacs(filepath):
    """Load PROTACs from TSV (tab-separated)."""
    protacs = []
    with open(filepath, "r") as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            # Tab-separated: name SMILES
            parts = line.split("\t")
            if len(parts) >= 2:
                name = parts[0]
                smiles = parts[1]
                label = "CHAM" if name in ["protac_1", "protac_2"] else "NON"
                mol = Chem.MolFromSmiles(smiles)
                if mol:
                    protacs.append({"name": name, "mol": mol, "label": label})
    return protacs


def load_benchmark(filepath):
    """Load benchmark from TSV (tab-separated)."""
    mols = []
    with open(filepath, "r") as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            parts = line.split("\t")
            if len(parts) >= 3:
                name, smiles, label = parts[0], parts[1], parts[2]
                mol = Chem.MolFromSmiles(smiles)
                if mol:
                    mols.append({"name": name, "mol": mol, "label": label})
    return mols


# Load molecules
log("Loading user PROTACs...")
protacs = load_user_protacs("chameleon_local/user_protacs.tsv")
log(f"Loaded {len(protacs)} PROTACs")

log("Loading benchmark molecules...")
bench_mols = load_benchmark("chameleon_local/benchmark.tsv")
log(f"Loaded {len(bench_mols)} benchmark molecules")
log("")

# Process all molecules
all_results = []

log("USER PROTAC RESULTS:")
log("-" * 80)
log(f"{'Name':<15} {'Label':<8} {'Promo':>8} {'Inhib':>8} {'Balance':>8} {'Score':>10}")
log("-" * 80)

for item in protacs:
    result = compute_fragment_score(item["mol"])
    result["name"] = item["name"]
    result["label"] = item["label"]
    all_results.append(result)

    log(
        f"{item['name']:<15} {item['label']:<8} {result['promoting_score']:>8.2f} "
        f"{result['inhibiting_score']:>8.2f} {result['balance']:>8.3f} {result['chameleon_score']:>10.4f}"
    )
    log(
        f"  PEG={result['prom_details']['peg_chain']}, "
        f"Amide={result['prom_details']['amide']}, "
        f"Ether={result['prom_details']['ether']}, "
        f"Alkyl5={result['inhib_details']['long_alkyl']}"
    )

# Process benchmark
for item in bench_mols:
    result = compute_fragment_score(item["mol"])
    result["name"] = item["name"]
    result["label"] = item["label"]
    all_results.append(result)

# Summary
cham_scores = [r["chameleon_score"] for r in all_results if r["label"] == "CHAM"]
non_scores = [r["chameleon_score"] for r in all_results if r["label"] == "NON"]

log("")
log("=" * 80)
log("SUMMARY STATISTICS")
log("=" * 80)
log("")

if cham_scores:
    log(
        f"Chameleonic (n={len(cham_scores)}): mean={np.mean(cham_scores):.4f}, std={np.std(cham_scores):.4f}"
    )
    log(f"  Range: [{np.min(cham_scores):.4f}, {np.max(cham_scores):.4f}]")

if non_scores:
    log(
        f"Non-chameleonic (n={len(non_scores)}): mean={np.mean(non_scores):.4f}, std={np.std(non_scores):.4f}"
    )
    log(f"  Range: [{np.min(non_scores):.4f}, {np.max(non_scores):.4f}]")

# PROTAC separation check
protac_results = {r["name"]: r for r in all_results if r["name"].startswith("protac_")}
if len(protac_results) >= 3:
    log("")
    log("PROTAC SEPARATION:")
    log("-" * 80)
    for name in ["protac_1", "protac_2", "protac_3"]:
        if name in protac_results:
            r = protac_results[name]
            log(f"  {name}: score={r['chameleon_score']:.4f} ({r['label']})")

    if all(n in protac_results for n in ["protac_1", "protac_2", "protac_3"]):
        cham_avg = np.mean(
            [
                protac_results["protac_1"]["chameleon_score"],
                protac_results["protac_2"]["chameleon_score"],
            ]
        )
        non_score = protac_results["protac_3"]["chameleon_score"]
        log(f"\n  Chameleonic avg: {cham_avg:.4f}")
        log(f"  Non-chameleonic: {non_score:.4f}")
        log(f"  Difference:      {cham_avg - non_score:+.4f}")
        if cham_avg > non_score:
            log("  VERDICT: SUCCESS - PEG-linked > Alkyl-linked")
        else:
            log("  VERDICT: FAIL - No separation")

log("")
log("=" * 80)
log("Experiment complete.")

# Save output file
output_text = "\n".join(output_lines)
with open("experiments/iter_22_output.txt", "w") as f:
    f.write(output_text)

# Save descriptors JSON
descriptors_clean = []
for r in all_results:
    clean = {
        k: float(v) if isinstance(v, (np.floating, np.integer)) else v
        for k, v in r.items()
        if k not in ["mol"]
    }
    descriptors_clean.append(clean)

with open("experiments/iter_22_descriptors.json", "w") as f:
    json.dump(descriptors_clean, f, indent=2)

log("\nResults saved to:")
log("  - experiments/iter_22_output.txt")
log("  - experiments/iter_22_descriptors.json")
